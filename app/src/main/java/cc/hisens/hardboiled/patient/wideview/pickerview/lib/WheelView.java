package cc.hisens.hardboiled.patient.wideview.pickerview.lib;
...
import cc.hisens.hardboiled.patient.wideview.pickerview.adapter.WheelAdapter;
import cc.hisens.hardboiled.patient.wideview.pickerview.listener.OnItemSelectedListener;
...
public class WheelView extends View {
    ...
    private int drawRightContentStart = 0;
    ...
    public WheelView(Context context, AttributeSet attrs) {
        ...
        labelTextSize = getResources().getDimensionPixelSize(R.dimen.pickerview_label_textsize);
//        customTextSize = getResources().getBoolean(R.bool.pickerview_customTextSize);
        ...
        if (attrs != null) {
            ...
            labelTextSize = a.getDimensionPixelOffset(R.styleable.wheelview_labelTextSize, labelTextSize);
            a.recycle();
        }
    ...
    private void initPaints() {
        ...
        paintLabel.setColor(textColorCenter);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (!TextUtils.isEmpty(label)) {
            if (drawRightContentStart == 0) {
                float maxWidth = 0;
                for (int i = 0; i < adapter.getItemsCount(); i++) {
                    String s1 = getContentText(adapter.getItem(i));
                    maxWidth = Math.max(maxWidth, paintCenterText.measureText(s1));
                }
                // 右边标识x坐标
                drawRightContentStart = drawCenterContentStart + getPaddingLeft() +
                        (int) maxWidth + (int) ScreenUtils.dp2px(getContext(), 8);
                // 防止label写出界
                drawRightContentStart = Math.min(drawRightContentStart, (int) (getWidth() - paintLabel.measureText(label)));
            }
            ...
            int textHeight = rect.height();
            float labelY = (measuredHeight + textHeight) * 0.5f;
            canvas.drawText(label, drawRightContentStart - CENTERCONTENTOFFSET, labelY, paintLabel);
        }
    }
